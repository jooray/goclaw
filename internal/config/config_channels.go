...
type ChannelsConfig struct {
	...
	SimpleX           SimpleXConfig            `json:"simplex"`
...
// SimpleXConfig configures the SimpleX Chat channel.
// Connects to a running SimpleX CLI instance via its WebSocket API.
type SimpleXConfig struct {
	Enabled        bool                `json:"enabled"`
	WebSocketURL   string              `json:"websocket_url"` // ws://127.0.0.1:5226
	AllowFrom      FlexibleStringSlice `json:"allow_from"`
	GroupPolicy    string              `json:"group_policy,omitempty"`     // "open" (default), "allowlist", "disabled"
	AllowedGroups  FlexibleStringSlice `json:"allowed_groups,omitempty"`   // group IDs (e.g. ["1"]) — when group_policy=allowlist
